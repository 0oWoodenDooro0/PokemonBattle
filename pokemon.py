...
class Pokemon:
    ...
    def attack(self, move: Move, defender_pokemon: 'Pokemon') -> MoveResult:
        print(f'{move.category=}')
        match move.category:
            case 0:
                move_result, damage = self.damage_attack(move, defender_pokemon)
            ...
            case 7:
                ...
                defender_pokemon.hp -= damage
                if self.stat_accuracy(move.stat_chance):
                    return self.stat_change(move, self, move_result)
                return move_result
            case 8:
                move_result, damage = self.damage_attack(move, defender_pokemon)
                defender_pokemon.hp -= damage
    ...
    def damage_attack(self, move: Move, defender_pokemon: 'Pokemon', move_result: MoveResult = None) -> tuple[MoveResult, int]:
        if move.crit_rate == 0:
            crit: int = self.basic_stats[6] // 2
        else:
            crit: int = self.basic_stats[6] * 4
        r: int = random.randint(1, 256)
        critical: int = 1 if r > crit else 2
        power: int = move.power
        attack_stat: int = self.stats[2] if move.damage_class == 2 else self.stats[4] if move.damage_class == 3 else 0
    ...
    def attack_accuracy(self, move: Move, defender_pokemon: 'Pokemon') -> bool:
        if not move.accuracy:
            return True
        b: int = math.floor(move.accuracy * 255 / 100)
        c: float = self.get_stat_effect(7)
